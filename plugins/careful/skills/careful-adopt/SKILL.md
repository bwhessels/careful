...
Inspect repository layout, build/test commands, dependencies, architecture clues, documentation, configuration, and recent changes. Mark discoveries as Verified or Inferred. Ask the user only about information that cannot be safely inferred and materially changes the workflow. Draft the smallest profile that explains commands, risk boundaries, and document locations; do not rewrite existing documentation merely to fit the harness. Add or merge tracked project guidance from `references/project-guidance.md` so Codex uses `careful-workflow` for substantive work when the plugin is installed.
