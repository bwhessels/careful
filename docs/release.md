...
For `public-intended` and `public` projects, the release owner must also complete the configured whole-repository public-readiness gate. For Careful itself, this is a maintainer review rather than formal independent review. Missing required artifacts or failed project checks block the gate; unresolved consequential decisions must be recorded rather than invented.
